...
public class IndexServiceImpl implements IndexService{
	...
	public List<TreeShow> finaCategoryTreeShow(List<Category> top, TreeShow tree) {
		...
		for(Category category : top){
			...
			List<Category> categoryChild = indexDao.finaCategoryById(new Long(category.getC_id()));
	...
	public List<Product> finaProductByLike() {
		List<Product> productList = new ArrayList<Product>();
			...
			int[] num = new int[5];
			for(int i = 0; i < ((productCount < num.length) ? productCount : num.length); i++){
...
/*	@Override
	public List<TreeShow> finaProductByFL(List<Category> top,
			TreeShow tree, int i) {
			...
			for(Category category : top){
				...
				if(categoryChild.size() == 0){
					List<Product> productList = indexDao.finaProductById(category.getC_id());
					treeShow.setChildProducts(productList);
				}else{
					List<TreeShow> categoryList2 = finaProductByFL(categoryChild , treeShow , i + 1);
					treeShow.setChildrens(categoryList2);
				}
				treeList.add(treeShow);
	...
	public List<TreeShow> finaProductByFL(List<Category> top,
			...
			for(Category category : top){
				...
				if(categoryChild.size() != 0){
					...
					List<Category> categoryChild1 = indexDao.finaCategoryById(new Long(category1.getC_id()));
					if(categoryChild1.size() != 0){
						...
						List<Product> productList = indexDao.finaProductById(category2.getC_id());
